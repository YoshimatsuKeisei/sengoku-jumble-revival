...
This proves where the current gate rectangles came from, but it does **not** prove that the original AVM1 collision/state-transition code used the same rectangles or an X-span test at all. Visible fence openings are SWF-derived visual evidence; converting their alpha bounds directly into behavioral admission geometry is a reconstruction choice unless AVM1 confirms it.

Runtime characterization now strengthens the implementation-side diagnosis. With two AI soldiers already in `EMERGENCY_RETREAT`, both assigned the same `TOP` gate and positioned inside the valid right edge of that gate, the current BattleScene ordering can produce the following sequence without manually moving either soldier outside the span: recovery targeting and movement keep both inside the gate span; `separateSoldiers` then pushes one sideways beyond the right-hand X limit; `resolveBaseAccessCollisions` immediately ejects that soldier from the friendly base rectangle; the soldier remains in `EMERGENCY_RETREAT` with the same `recoveryGate`, so the next recovery update targets the same gate again. This directly demonstrates a reconstruction-side loop of `enter -> crowd separation -> span loss -> ejection -> retarget same gate`.

The same test also exposes a reconstruction-specific left/right asymmetry. On the player base's left edge, the battlefield boundary clamp in `separateSoldiers` prevents the soldier center from moving below `SOLDIER_RADIUS`, which can keep it inside the gate's left admissible center boundary. The gate's right edge has no equivalent battlefield clamp, so the same crowd separation can move a soldier outside the admissible X span there. This asymmetry is evidence about the current runtime, not evidence about original SWF behavior.

There is also no distinct runtime state for "gate entry in progress." `recoveryGateEntered` is initialized false and only becomes true inside `enterHealing`, after `hasClearedBaseGateBoundary` has already succeeded and the soldier is moved to a healing slot. Therefore an `EMERGENCY_RETREAT` soldier that has started crossing the gate is still judged purely by its instantaneous X coordinate on every subsequent base-access pass. A post-movement separation can revoke admission before the healing transition. Adding a separate entry-progress latch, widening the gate span, or exempting all retreating soldiers once inside the base could each remove the reconstruction loop, but those are behavior changes and must not be selected as SWF-conformance fixes until original gate-entry semantics are recovered.

Accordingly, `FRIENDLY_BASE_RETREAT_GATE_CONGESTION` remains `inferred`. Its machine-readable record now captures the directly reproduced runtime facts (`runtimeCrowdSeparationCanEjectEnteringRetreater`, `runtimeEjectedRetreaterRetargetsSameGate`, `runtimeGateEdgeBehaviorIsAsymmetric`, and the absence of a distinct gate-entry-in-progress state), while `swfBehavioralAdmissionSpan` remains null and the exact SWF admission/order semantics remain required before redesigning recovery behavior.
