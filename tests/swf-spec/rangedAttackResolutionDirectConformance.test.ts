...
function ranged(id: string, technique: UnitTechnique, sourceX = 500, sourceY = 500): Soldier {
  const soldier = unit(id, "player", sourceX, sourceY);
  soldier.unitType = technique.startsWith("TEPPOU_") ? "TEPPOU" : "ARCHER";
  soldier.technique = technique;
  return soldier;
}

describe("direct raw-SWF ranged attack resolution safe probe", () => {
  it("records latch, synchronous resolution, pre-effect order, and explicitly deferred displacement", () => {
    ...
    const synchronousRule = combatSpec.rules.find((rule) => rule.id === "RANGED_SYNCHRONOUS_GAMEPLAY_RESOLUTION");
    expect(synchronousRule?.expected).toMatchObject({
    ...
    expect(synchronousRule && "implementation" in synchronousRule ? synchronousRule.implementation : null).toMatchObject({
      status: "partial_probe",
      deferred: ["defender_10_source_unit_decaying_fx_fy_displacement"],
    });
  ...
  it("centers the no-l scan on raw _x+x rather than the current source cell", () => {
    ...
    // ARCHER_ARROW has a three-cell raw range window. With the predicted origin,
    // source cell 17 enters the scan where the current-position-only origin would miss it.
  ...
  it("applies fire-arrow fire before a guarded direct arrow, while primary KATON removes only that pre-effect", () => {
    ...
    const fire2 = ranged("fire2", "ARCHER_FIRE_ARROW", 500, 600);
  ...
  it("gives guarded ranged attacks the confirmed ten-tick response without importing the rejected movement impulse layer", () => {
    ...
    expect({ x: target.x, y: target.y }).toEqual(before);
  });

  it.todo("applies the confirmed raw 10-source-unit decaying ranged fx/fy displacement in its own isolated visual-regression probe");
});
