...
describe("Phase 4F ninja", () => {
  ...
  it("uses the confirmed centered 3x3 rectangle", () => {
    ...
    expect(isInsideNinjaForwardSector(attacker, side)).toBe(true); expect(isInsideNinjaForwardSector(attacker, rear)).toBe(true);
    expect(findNinjaTargets(attacker, [attacker, front, up, down, side, rear]).map((s) => s.id)).toEqual(["f", "u", "d", "s", "r"]);
  });
  it("makes shadow run deeper and stronger than ninjutsu, both above prototype", () => {
    expect(getNinjaDashDistance("NINJA_NINJUTSU")).toBeGreaterThan(0);
  ...
  it("applies MIGHT and defense to ninjutsu but not bypass techniques", () => {
    ...
    executeNinjaAttack(attacker, [attacker, guarded], [], [], 0, () => 0, "GENERAL_FORCED"); expect(guarded.hp).toBe(guarded.maxHp); expect(guarded.combatFeedbackMarker).toBe("S");
    ...
    const event = executeNinjaAttack(genjutsu, [genjutsu, victim], [], [], 0, () => 0, "GENERAL_FORCED")!;
    expect(victim.hp).toBe(victim.maxHp - 2); expect(victim.combatFeedbackMarker).toBe("H"); expect(victim.isConfused).toBe(true); expect(event.victimIds).toEqual(["v"]);
  });
  ...
  it("keeps the barrier 85% support and 15% ninja-charge branches exclusive", () => {
    ...
    const enemy = createSoldier("e", "enemy", "ai", 140, 100); const event = executeNinjaAttack(barrier, [barrier, ally, otherBarrier, enemy], [], [], 0, () => 0, "GENERAL_FORCED")!;
    expect(ally.hp).toBeGreaterThan(ally.maxHp - 10); expect(otherBarrier.hp).toBeGreaterThan(otherBarrier.maxHp - 10);
    expect(barrier.hp).toBeGreaterThan(barrier.maxHp - 10); expect(ally.specialReadyAt).toBe(99_999);
    expect(ally.temporaryOrder).toBeNull(); expect(event.forcedEvents).toEqual([]);

    const chargeBarrier = createSoldier("cb", "player", "ai", 100, 100, "charge", undefined, ninja("NINJA_BARRIER"));
    const chargeAlly = createSoldier("cn", "player", "ai", 105, 100, "charge", undefined, ninja());
    const strategy = chargeAlly.strategy;
    const chargeEvent = executeNinjaAttack(chargeBarrier, [chargeBarrier, chargeAlly], [], [], 0, () => 0.9, "GENERAL_FORCED")!;
