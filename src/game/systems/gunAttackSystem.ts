import { DEFENSE_CONFIG, GUN_CONFIG, SPECIAL_ABILITY_CONFIG, SPECIAL_ATTACK_CONFIG } from "../config";
...
export function getGunRange(technique: UnitTechnique): number | null {
  if (technique === "TEPPOU_SHOOTING") return GUN_CONFIG.shootingRange;
  if (technique === "TEPPOU_SNIPING") return GUN_CONFIG.snipingRange;
  if (technique === "TEPPOU_BOMBARDMENT") return GUN_CONFIG.shootingRange;
  return null;
}
...
export function getGunMovementDecision(soldier: Soldier, target: Soldier): GunMovementDecision {
  ...
  if (distance <= soldier.attackRange) return "NORMAL_COMBAT";
...
export function executeGunAttack(attacker: Soldier, target: Soldier, currentTime: number,
  ...
  const guarded = isDamageGuarded(target, "GUN_ATTACK", random);
  ...
  const bombardmentComponents = bombardment ? applyRareDamageImmunity(target, {
    ...
    DIRECT_SPECIAL: BOMBARDMENT_DAMAGE_COMPONENTS.DIRECT_SPECIAL! + Number(hasSpecialAbility(attacker, "MIGHT")),
  }) : null;
  const primaryDamage = bombardment ? totalDamageComponents(bombardmentComponents!) : calculateGunDirectDamage(attacker);
  if (!guarded) { applyDamage(target, primaryDamage); queueMoutaiOnDamage(target, primaryDamage); }
  target.combatFeedbackMarker = guarded ? "S" : "H";
  target.combatFeedbackUntil = currentTime + DEFENSE_CONFIG.guardMarkerDurationMs;
  if (!guarded && !target.isDead) startHitReaction(target, attacker, currentTime, 0);
  ...
  if (bombardment && !guarded) {
    ...
    for (const splash of soldiers) {
      if (splash === target || splash.team === attacker.team || splash.isDead || splash.hp <= 0
        ...
        || Math.hypot(splash.x - impactX, splash.y - impactY) > SPECIAL_ATTACK_CONFIG.radius) continue;
      if (isDamageGuarded(splash, "GUN_ATTACK", random)) continue;
